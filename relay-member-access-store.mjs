...
const RELAY_MEMBER_ACCESS_FILENAME = 'relay-member-access-cache.json';
...
function storePath() {
  return join(resolveStorageBase(), RELAY_MEMBER_ACCESS_FILENAME);
}
...
function normalizeEntry(entry, now = Date.now()) {
  ...
  const relayKey = normalizeIdentity(entry.relayKey);
  const publicIdentifier = normalizeIdentity(entry.publicIdentifier);
  ...
  return {
    ...
    gatewayId,
    authMethod:
      typeof entry.authMethod === 'string' && entry.authMethod.trim()
        ? entry.authMethod.trim()
        : 'relay-scoped-bearer-v1',
    accessToken,
    grantId:
      typeof entry.grantId === 'string' && entry.grantId.trim()
        ? entry.grantId.trim()
        : null,
    scopes: normalizeScopes(entry.scopes),
    subjectPubkey: normalizeIdentity(entry.subjectPubkey),
    subjectRole:
      typeof entry.subjectRole === 'string' && entry.subjectRole.trim()
        ? entry.subjectRole.trim()
        : 'member',
    sponsorPubkey: normalizeIdentity(entry.sponsorPubkey),
    devicePeerKey:
      typeof entry.devicePeerKey === 'string' && entry.devicePeerKey.trim()
        ? entry.devicePeerKey.trim()
        : null,
    issuedAt: Number.isFinite(Number(entry.issuedAt)) ? Number(entry.issuedAt) : null,
    expiresAt: Number.isFinite(Number(entry.expiresAt)) ? Number(entry.expiresAt) : null,
    refreshAfter: Number.isFinite(Number(entry.refreshAfter)) ? Number(entry.refreshAfter) : null,
...
function mergeEntries(left, right, now = Date.now()) {
  ...
  return {
    ...
    authMethod: newer.authMethod || older.authMethod || 'relay-scoped-bearer-v1',
    accessToken: newer.accessToken || older.accessToken || null,
    grantId: newer.grantId || older.grantId || null,
    scopes: newer.scopes?.length ? newer.scopes : (older.scopes || []),
    subjectPubkey: newer.subjectPubkey || older.subjectPubkey || null,
    subjectRole: newer.subjectRole || older.subjectRole || 'member',
    sponsorPubkey: newer.sponsorPubkey || older.sponsorPubkey || null,
    devicePeerKey: newer.devicePeerKey || older.devicePeerKey || null,
    issuedAt: newer.issuedAt || older.issuedAt || null,
    expiresAt: newer.expiresAt || older.expiresAt || null,
    refreshAfter: newer.refreshAfter || older.refreshAfter || null,
...
function pruneEntry(entry, now = Date.now()) {
  ...
  if (normalized.expiresAt && normalized.expiresAt <= now) return null;
...
async function loadStore() {
  ...
  try {
    ...
    if (error?.code !== 'ENOENT') {
      configuredLogger?.warn?.('[Worker] Failed to load relay member access cache', {
        path,
...
function scheduleFlush() {
  ...
  flushTimer = setTimeout(() => {
    ...
    flushStore().catch((error) => {
      configuredLogger?.warn?.('[Worker] Failed to flush relay member access cache', {
        error: error?.message || error
      });
    });
  }, 1200);
...
export function configureRelayMemberAccessStore({ storageBase = null, logger = null } = {}) {
...
export async function upsertRelayMemberAccess(entry = {}) {
  await loadStore();
  const now = Date.now();
  ...
  const keys = identityKeys(update);
  if (!keys.size) return null;

  let mergedResult = null;
  for (const key of keys) {
    const merged = mergeEntries(entries.get(key) || null, update, now);
    if (merged) {
...
export async function getRelayMemberAccess({ relayKey = null, publicIdentifier = null } = {}) {
  await loadStore();
...
export async function removeRelayMemberAccess({ relayKey = null, publicIdentifier = null } = {}) {
  await loadStore();
  const keys = identityKeys({ relayKey, publicIdentifier });
...
export async function pruneRelayMemberAccessStore(now = Date.now()) {
  await loadStore();
  let changed = false;
  for (const [key, value] of entries.entries()) {
    const pruned = pruneEntry(value, now);
